...
#ifndef KERNEL_INCLUDE_KERNEL_KEYBOARD_H_
#define KERNEL_INCLUDE_KERNEL_KEYBOARD_H_
/*keyboard MARCO*/
...
/* keyboard buffer */
struct s_kb {
	unsigned char* p_head;   //buffer next empty byte
	unsigned char* p_tail;   //next handle byte
	int count;      //how many byte in buffer
	unsigned char buf[KB_IN_BYTES];
};

typedef struct s_kb KB_BUFF;
...
8	09		89			PAUSE 		E1,1D,45	-NONE-E1,9D,C5
-----------------
ACPI Scan Codes:
-------------------------------------------
...
class KeyboardInterruptHandler : public AbstractInterruptHandler {
public:
...
#endif /* KERNEL_INCLUDE_KERNEL_KEYBOARD_H_ */
